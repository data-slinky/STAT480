...
avgArrDelayByManufacturer
avgDepDelayByManufacturer
...
treemap(modelByAircraftType[modelByAircraftType$aircraft_type=="Fixed Wing Multi-Engine", ], 
        index = c("aircraft_type", "model"),
        vSize = "model_count", 
        type = "index")
treemap(modelByAircraftType[modelByAircraftType$aircraft_type=="Fixed Wing Single-Engine", ], 
        index = c("aircraft_type", "model"),
        vSize = "model_count", 
        type = "index")
treemap(modelByAircraftType[modelByAircraftType$aircraft_type=="Rotorcraft", ], 
        index = c("aircraft_type", "model"),
        vSize = "model_count", 
        type = "index")
        #title = "Tree Map of Model Counts Grouped by Aircraft Types")
counts = table(airlines_delay$model, airlines_delay$aircraft_type)
```
I separated the treemaps according to the aircraft type because otherwise fixed wing multi-engine would make the other two aircraft types too hard to see. The most popular model of fixed wing multi-engine is the MD-88; the most popular model of fixed wing single-engine is the FALCON XP. As for rotorcraft, only one model is found: 206B.  
